...
const Pricing = () => {
  ...
  useEffect(() => {
    ...
    // Helper to apply a bundle
    interface ApplyBundleParams {
      bundleName: string;
      bundlePrice: number;
    ...
    function applyBundle(
      ...
      setCart([
      ...cart.filter((item: CartItem) => !replacedItems.includes(item) && item.type !== 'bundle'),
  ...
  interface CalculateCartTotalItem {
    price: number;
    [key: string]: any;
  }

  const calculateCartTotal = (cartItems: CalculateCartTotalItem[]): number => {
    return cartItems.reduce((sum: number, item: CalculateCartTotalItem) => sum + item.price, 0);
  };
