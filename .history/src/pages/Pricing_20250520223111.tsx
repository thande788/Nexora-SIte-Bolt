...
const Pricing = () => {
  ...
  const [bundleApplied, setBundleApplied] = useState<{ name: string; message: string; icon: React.ReactNode } | null>(null);
  const [bundleToast, setBundleToast] = useState<string | null>(null);
  const navigate = useNavigate();

  // Helper to redirect to contact with prefilled message
  const handleAddToPlan = (planName: string) => {
    // You can pass state or query params; here we use query params
  ...
  // Helper to get add-ons from cart
  const getAddOns = () => cart.filter((item: any) => item.type === 'addon');
                  ...
                  >
                    ...
                    <div className="p-6 pt-2 mt-auto flex flex-col gap-2">
                      <button
                        ...
                        onClick={() => handleAddToPlan(plan.name)}
                      >
                        Add to Plan
                      </button>
                    </div>
                ...
                >
                  ...
                  <div className="p-6 pt-2 mt-auto flex flex-col gap-2">
                    <button
                      ...
                      onClick={() => handleAddToPlan(bundle.name)}
                    >
                      Add to Plan
                    </button>
                  </div>
              ...
              {addons.map((addon) => (
                ...
                >
                  ...
                  <div className="p-6 pt-2 mt-auto flex flex-col gap-2">
                    <button
                      className="w-full py-2 px-4 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition"
                      onClick={() => handleAddToPlan(addon.name)}
                    >
                      Add to Plan
                    </button>
                  </div>
