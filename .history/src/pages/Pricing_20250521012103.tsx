...
const Pricing = () => {
  ...
  const addToCart = (item: CartItem, type: 'service' | 'bundle' | 'addon') => {
    setShowCart(true);
    setCartWarning(null);

    // Only one base plan (service or bundle)
    if (type === 'service') {
      if (cart.some(i => i.type === 'bundle')) {
        setCartWarning('Remove the bundle before adding a service.');
        return;
      }
      // Only one service per category (optional, if you want)
      // ...category logic here if needed...
      setCart([
        ...cart.filter(i => i.type !== 'service'),
        { ...item, type }
      ]);
    } else if (type === 'bundle') {
      if (cart.some(i => i.type === 'service')) {
        setCartWarning('Remove services before adding a bundle.');
        return;
      }
      setCart([
        ...cart.filter(i => i.type !== 'bundle'),
        { ...item, type }
      ]);
    } else if (type === 'addon') {
      ...
      if (cart.some(i => i.name === item.name && i.type === 'addon')) {
        setCartWarning('This add-on is already in your plan.');
        return;
      }
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    e.preventDefault();
    if (!getBasePlan()) {
      setCartWarning('Please choose a base plan or bundle.');
      return;
    }
  ...
  return (
    <div className="bg-white dark:bg-gray-900">
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl mx-auto text-center">
            <div className="flex flex-col sm:flex-row items-center justify-center gap-2 mb-6">
              <span className="inline-block px-2 py-1 rounded-full bg-white/20 text-white text-xs sm:text-sm font-medium">
                Transparent Pricing
              </span>
              <span className="inline-block bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs font-semibold sm:ml-2 mt-2 sm:mt-0 text-center w-full sm:w-auto">
      ...
      <section className="bg-white dark:bg-gray-900 sticky top-0 z-10 border-b border-gray-100 dark:border-gray-800">
        <div className="container mx-auto px-2 sm:px-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-4 py-4">
          {/* Make tabs horizontally scrollable on mobile */}
          <div className="w-full md:w-auto overflow-x-auto">
            <div className="flex space-x-2 md:space-x-4 min-w-max">
              <button
                className={clsx(
                  'px-3 py-2 rounded-lg font-semibold transition text-sm sm:text-base',
                  tab === 'services'
              ...
              >
                <PieChart className="inline mr-1" size={16} /> Services
              </button>
              <button
                className={clsx(
                  'px-3 py-2 rounded-lg font-semibold transition text-sm sm:text-base',
              ...
              >
                <Layers className="inline mr-1" size={16} /> Bundles
              </button>
              <button
                className={clsx(
                  'px-3 py-2 rounded-lg font-semibold transition text-sm sm:text-base',
              ...
              >
                <Star className="inline mr-1" size={16} /> Enterprise
              </button>
              <button
                className={clsx(
                  'px-3 py-2 rounded-lg font-semibold transition text-sm sm:text-base',
              ...
              >
                <Zap className="inline mr-1" size={16} /> Add-ons
              </button>
            </div>
          </div>
        ...
        {cart.length > 0 && (
          <div className="w-full">
            ...
            <div className="flex justify-between font-semibold mt-2">
              <span>Total:</span>
              <span>KES {calculateCartTotal(cart).toLocaleString()}</span>
            </div>
            <form
              name="nexora-subscribe"
              method="POST"
            ...
            >
              ...
              <button
                type="submit"
                className="w-full py-2 px-4 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition disabled:opacity-50 mt-2"
                disabled={!getBasePlan()}
              >
                Subscribe
              </button>
            </form>
            <button
              className="w-full mt-2 py-2 px-4 bg-red-50 hover:bg-red-100 text-red-700 font-semibold rounded-lg transition"
              onClick={clearCart}
            >
              Clear Cart
            </button>
            {cartWarning && (
              <div className="mt-2 text-xs text-red-600 bg-red-50 dark:bg-red-900/30 rounded px-2 py-1">
                {cartWarning}
              </div>
            )}
            {!getBasePlan() && (
              <div className="mt-2 text-xs text-yellow-700 bg-yellow-50 dark:bg-yellow-900/30 rounded px-2 py-1">
                Please choose a base plan or bundle.
              </div>
            )}
          </div>
        )}
      ...
      {/* --- Responsive Cart UI (sticky on desktop, modal/drawer on mobile) --- */}
