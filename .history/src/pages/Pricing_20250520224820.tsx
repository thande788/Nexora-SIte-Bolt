...
// TODO: Uncomment the line below after creating CartTable.tsx in the same directory
// import CartTable from "./CartTable";
...
const Pricing = () => {
  ...
  const applyBundle = (bundleName, bundlePrice, replaceItems) => {
    const filtered = cart.filter((i) => !replaceItems.some((r) => r.name === i.name));
    const bundleItem = { name: bundleName, price: bundlePrice, type: "bundle" };
  ...
  useEffect(() => {
    ...
    const has = (names) => names.every((n) => cart.some((i) => i.name === n));
