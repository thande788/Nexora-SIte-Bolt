...
const Pricing: React.FC = () => {
  ...
  const addToCart = (item: PlanItem) => {
    // Prevent duplicates and overlapping base/bundle
    if (cart.some((i) => i.name === item.name)) return;
    if (item.type === "bundle") {
      // Remove all base services and bundles before adding new bundle
      setCart((prev) => [
        ...prev.filter((i) => i.type !== "service" && i.type !== "bundle"),
        item,
      ]);
    } else if (item.type === "service") {
      // Remove bundles before adding base service
      setCart((prev) => [
  ...
  // Only allow subscribe if exactly one base plan or bundle is selected
              ...
              <Disclosure key={plan.name}>
                {({ open }) => (
                  <div className="rounded-xl border bg-white dark:bg-gray-800 shadow-soft">
                    <Disclosure.Button className="w-full flex justify-between items-center px-6 py-4 text-left">
            ...
            {/* <div className="rounded-xl border bg-white dark:bg-gray-800 shadow-soft">
              <div className="px-6 py-4">
                <h4 className="font-semibold text-lg mb-2">
                  {cat.name} - Custom Plan
                </h4>
                <p className="text-sm text-gray-500 mb-4">
                  Tailored solutions to fit your business needs.
                </p>
                <Link href="/contact">
                  <a className="block text-center bg-primary-600 text-white rounded-lg py-2 font-semibold transition hover:bg-primary-700">
                    Contact Sales
                  </a>
                </Link>
              </div>
            ...
            <div className="rounded-xl border bg-white dark:bg-gray-800 shadow-soft">
              ...
              <Disclosure.Panel className="px-6 pb-4">
                <PricingCard
                  ...
                  period={addon.period}
                  features={[{ text: "Modular add-on", included: true }]}
                  ctaText={
  ...
  const planSummary = (
    <div className="sticky top-24 bg-white dark:bg-gray-900 border border-primary-200 dark:border-gray-700 rounded-xl shadow-xl p-6 mb-8">
      <h3 className="font-bold text-lg mb-2 flex items-center gap-2">
        <span role="img" aria-label="cart">🛒</span> Plan Summary
      </h3>
      <ul className="mb-2">
        {cart.map((item) => (
