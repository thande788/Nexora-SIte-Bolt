...
const Pricing = () => {
  ...
  // Add to cart logic for services: only one per category, but can combine across categories
  const addToCart = (item: any, type: 'service' | 'bundle' | 'addon') => {
    setShowCart(true);
    if (type === 'service') {
      setCart((prev) => {
        // Remove any existing service from the same category
      ...
      // Only one bundle at a time, remove all services/bundles
      setCart((prev) => [
            ...
            {cart.length === 0 ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between font-bold text-base mt-2 mb-3">
                  <span>Total</span>
                  <span className="text-primary-700 dark:text-primary-200 text-base">
