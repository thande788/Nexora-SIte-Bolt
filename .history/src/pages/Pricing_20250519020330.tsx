...
const Pricing = () => {
  ...
  const [showCart, setShowCart] = useState(false); // NEW: controls cart widget visibility
  ...
  const addToCart = (item: any, type: 'service' | 'bundle' | 'addon') => {
    setShowCart(true); // Show cart when user adds
    if (type === 'service' || type === 'bundle') {
      setCart((prev) => [
        ...prev.filter((i) => i.type === 'addon'),
        { ...item, type }
      ]);
    } else {
      setCart((prev) =>
  ...
  return (
    <div className="bg-white dark:bg-gray-900">
      ...
      {/* Cart/Plan Summary Widget */}
      {showCart && (
        <div className={clsx(
          "fixed z-40 transition-all duration-300",
          "bottom-6 left-1/2 transform -translate-x-1/2", // Centered at bottom
          "w-full max-w-md"
        )}>
          <div className="bg-gradient-to-br from-primary-50 to-white dark:from-gray-800 dark:to-gray-900 border border-primary-200 dark:border-gray-700 rounded-2xl shadow-2xl p-6 relative animate-fade-in">
            <button
            ...
            <h3 className="font-bold text-xl mb-3 text-primary-800 dark:text-white flex items-center gap-2">
              <Layers className="text-primary-600" size={22} /> Your Plan
            </h3>
            {cart.length === 0 ? (
              <div className="text-gray-500 text-sm text-center py-8">No items selected.</div>
            ) : (
              <>
                <ul className="mb-4 divide-y divide-primary-100 dark:divide-gray-700">
                ...
                <div className="flex items-center justify-between font-bold text-lg mt-2 mb-4">
                  <span>Total</span>
                  <span className="text-primary-700 dark:text-primary-200">
                    KES {Math.round(total).toLocaleString()}{billing === 'annual' ? ' /mo (annual)' : ' /mo'}
                  </span>
                </div>
                <button
                  className="w-full mb-3 py-2 px-4 bg-red-100 hover:bg-red-200 text-red-700 font-semibold rounded-lg transition"
                  onClick={clearCart}
                >
                  Clear Cart
                </button>
