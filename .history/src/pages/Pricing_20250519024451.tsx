...
const Pricing = () => {
  ...
  const addToCart = (item: any, type: 'service' | 'bundle' | 'addon') => {
    ...
    if (type === 'service') {
      // If a bundle is already selected, warn and do not add
      if (cart.some(i => i.type === 'bundle')) {
        setCartWarning('You cannot add a service when a bundle is selected. Remove the bundle first.');
        return;
      }
      ...
      // If a service is already selected, warn and do not add
      if (cart.some(i => i.type === 'service')) {
        setCartWarning('You cannot add a bundle when a service is selected. Remove the service(s) first.');
        return;
      }
  ...
  // Calculate total: base plan + all add-ons
  const total = (() => {
    let sum = 0;
    const base = getBasePlan();
    if (base) {
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    ...
    // Submit logic (Netlify Forms/EmailJS)
  ...
  return (
    <div className="bg-white dark:bg-gray-900">
      ...
      {/* Cart/Plan Summary */}
      {showCart && (
        <div className="fixed bottom-4 left-4 md:sticky md:top-32 md:self-start md:w-80 z-40">
          <div className="bg-white dark:bg-gray-900 border border-primary-200 dark:border-gray-700 rounded-xl shadow-xl p-4 relative animate-fade-in md:sticky md:top-32">
            ...
            {cartWarning && (
              <div className="mb-2 text-xs text-red-600 bg-red-50 dark:bg-red-900/30 rounded px-2 py-1">{cartWarning}</div>
            )}
            {!getBasePlan() && (
              <div className="mb-2 text-xs text-yellow-700 bg-yellow-50 dark:bg-yellow-900/30 rounded px-2 py-1">
                Please choose a base plan.
              </div>
            )}
            {cart.length === 0 ? (
              <div className="text-gray-500 text-sm text-center py-6">No items selected.</div>
            ) : (
              <>
                <ul className="mb-3 divide-y divide-primary-100 dark:divide-gray-700">
                  {getBasePlan() && (
                    <li className="flex items-center justify-between py-2">
                      <span className="font-semibold truncate">{getBasePlan().name}</span>
                      <span className="flex items-center gap-2">
                        <span className="text-xs text-primary-700 dark:text-primary-200 font-mono">
                          {getBasePlan().price.toLocaleString()}
                          <span className="ml-1 text-xs text-gray-500 dark:text-gray-400 font-normal">
                            {getBasePlan().period === 'one-time' ? 'one-time' : '/month'}
                          </span>
                        </span>
                        <button onClick={() => removeFromCart(getBasePlan().name, getBasePlan().type)} className="ml-1 text-gray-400 hover:text-red-500">
                          <X size={12} />
                        </button>
                      </span>
                    </li>
                ...
                <div className="flex items-center justify-between font-bold text-base mt-2 mb-3">
                  <span>Total</span>
                  <span className="text-primary-700 dark:text-primary-200 text-base font-mono">
