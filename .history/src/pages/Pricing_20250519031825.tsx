...
const Pricing = () => {
  ...
  // --- Cart logic: only one base plan (service(s) or bundle) + all add-ons ---
  ...
  // Calculate the cart total, respecting bundle logic
  function calculateCartTotal(selectedItems: CartItem[]): number {
    // If a bundle is present, ignore any overlapping services (shouldn't be present, but for safety)
    const bundle = selectedItems.find(i => i.type === 'bundle');
    if (bundle) {
      // Only add-ons and the bundle
      return selectedItems
        .filter(i => i.type === 'addon' || i.type === 'bundle')
        .reduce((sum, item) => sum + item.price, 0);
    }
    // Otherwise, sum all services and add-ons
